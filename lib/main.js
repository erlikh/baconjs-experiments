...
let tick = Bacon.interval(1000, true);

function plusArr(first, second) {
  return [first[0]+second[0], first[1]+second[1]]
}

function handleTick(prev) {
  return prev
}

function handleTickDir(prev, _, dir) {
  return plusArr(prev, dir);
}

let pos = Bacon.update(
  [0, 0],
  [tick, dir], handleTickDir,
  [tick], handleTick
);
...
let $hero = $("#hero");
pos.map(multiplyArr(multiplier)).onValue(function(val) {
  $hero.attr("style", `left: ${val[0]}px; top: ${val[1]}px`);
});
